...
__device__ typename vec3<float>::Type
bodyBodyInteraction_float(typename vec3<float>::Type ai,
                    typename vec4<float>::Type bi,
                    typename vec4<float>::Type bj)
{
    typename vec3<float>::Type r;
    ...
    distSqr += getSofteningSquared<float>();
...
template <typename T>
__device__ typename vec3<float>::Type
computeBodyAccel_float(typename vec4<float>::Type bodyPos,
                 typename vec4<T>::Type *positions,
                 int numTiles, cg::thread_block cta)
{
    typename vec4<float>::Type *sharedPos = SharedMemory<typename vec4<float>::Type>();

    typename vec3<float>::Type acc = {0.0f, 0.0f, 0.0f};

    for (int tile = 0; tile < numTiles; tile++)
    {
        sharedPos[threadIdx.x].x = positions[tile * blockDim.x + threadIdx.x].x;
        sharedPos[threadIdx.x].y = positions[tile * blockDim.x + threadIdx.x].y;
        sharedPos[threadIdx.x].z = positions[tile * blockDim.x + threadIdx.x].z;
        sharedPos[threadIdx.x].w = positions[tile * blockDim.x + threadIdx.x].w;
        cg::sync(cta);
...
#pragma unroll 128
        ...
        for (unsigned int counter = 0; counter < blockDim.x; counter++)
        {
            acc = bodyBodyInteraction_float(acc, bodyPos, sharedPos[counter]);
        }
                ...
                float deltaTime, float damping, int numTiles)
{
    ...
    typename vec4<float>::Type position_float;
    typename vec4<T>::Type position = oldPos[deviceOffset + index];
    position_float.x = oldPos[deviceOffset + index].x;
    position_float.y = oldPos[deviceOffset + index].y;
    position_float.z = oldPos[deviceOffset + index].z;
    position_float.w = oldPos[deviceOffset + index].w;
/*
    typename vec3<T>::Type accel = computeBodyAccel<T>(position,
                                                       oldPos,
                                                       numTiles, cta);
*/
    typename vec3<float>::Type accel = computeBodyAccel_float<T>(position_float,
                                                       oldPos,
                                                       numTiles, cta);
