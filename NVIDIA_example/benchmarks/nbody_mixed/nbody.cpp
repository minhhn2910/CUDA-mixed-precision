...
class NBodyDemo
{
    public:
        ...
        static bool calculate_relative_err(int numBodies)
        {
            return m_singleton->_calculate_relative_err(numBodies);
        }
    ...
    private:
        ...
        double _calculate_relative_err(int numBodies)
        {
           // assert(m_nbodyCuda);

            double error = 0.0;
	    int fail_ = 0;
	    //m_nbody->update(0.01f);
            m_nbody->update(0.001f);

            {
				...
				fail_ ++;
                    //	   passed = false;
                    //    printf("Error: index %d (host)%f != (device)%f\n",i, cpuPos[i], cudaPos[i]);
                    }

                }
            }

             printf("average abs error %lf\n", error/numBodies);
	     printf("percent passed bodies %f\n",(numBodies-fail_)/float(numBodies));
            return error/numBodies;
        }
...
main(int argc, char **argv)
{
    ...
    if (fp64)
    {
        ...
        else if (compareToCPU)
        {
            ...
            bTestResults = true;
            NBodyDemo<double>::calculate_relative_err(numBodies);
        }
        else
        {
    ...
    else
    {
        ...
        else if (compareToCPU)
        {
           // bTestResults = NBodyDemo<float>::compareResults(numBodies);
              bTestResults = true;
              NBodyDemo<float>::calculate_relative_err(numBodies);
        }
        else
        {
            glutDisplayFunc(display);
