...
__global__ void nrpol3_kernel(float *A_coeff, float *B_coeff, float *C_coeff, float *D_coeff, float *x0_in, float *root, int size, float err_thresh, int speed_threshold)
{

	int blockId = blockIdx.x + blockIdx.y * gridDim.x;

#ifdef SCATTER
   if(blockId %100 < speed_threshold){
#else
   if(blockId < speed_threshold){
#endif
//	if(blockId < speed_threshold){ //from 0-Blocknumber
	...
	if(idx < size)
	{
		half2 curr_err = __float2half2_rn(MAX_DIFF * err_thresh);

		half2 x0 = __floats2half2_rn(x0_in[idx],x0_in[idx+1]);

		half2 x1, fx, fdx;

		half2 A_coeff_temp = __floats2half2_rn(A_coeff[idx],A_coeff[idx+1]);
		half2 B_coeff_temp = __floats2half2_rn(B_coeff[idx],B_coeff[idx+1]);
		half2 C_coeff_temp = __floats2half2_rn(C_coeff[idx],C_coeff[idx+1]);
		half2 D_coeff_temp = __floats2half2_rn(D_coeff[idx],D_coeff[idx+1]);

	//	float temp_result = 0;//store temp_res in loop

		half2 temp_result;// = __float2half2_rn(0.0);
		half2 temp_err;

		for (int i = 0; i < MAX_LOOP; i++) {
			//fx  = F(x0);
			fx = (A_coeff_temp*x0*x0*x0)+(B_coeff_temp*x0*x0)+(C_coeff_temp*x0)+D_coeff_temp;
			//fdx = FD(x0);
			fdx = (3.0*A_coeff_temp*x0*x0)+(2.0*B_coeff_temp*x0)+(C_coeff_temp);
			//x1  = x0 - (fx/fdx);
			x1 = x0 - fx*fast_h2rcp(fdx);

			//temp_err = h2absf((x1-x0) / x1);
#if __CUDA_ARCH__ == 700 
			temp_err = h2absf((x1-x0)* fast_h2rcp(x1));
#else
      temp_err = h2absf((x1-x0)* h2rcp(x1));
#endif

			if (temp_err.x < curr_err.x) {
				curr_err.x = temp_err.x ;
				temp_result.x = x1.x ;
			}
			if (temp_err.y < curr_err.y) {
				curr_err.y = temp_err.y ;
				temp_result.y = x1.y ;
			}
	/*
	...
	*/
	...
	//#pragma parrot(output, "nrpol3_kernel", [1]<-2.0; 2.0>parrotOutput)
		float2 result = __half22float2(temp_result);
		root[idx] = result.x;
		root[idx+1] = result.y;
...
int main(int argc, char* argv[])
{
	if(argc != 5)
	{
		std::cerr << "Usage: ./nrpoly3.out <input file coefficients> <output file> <error threshold> speed_threshold" << std::endl;
		exit(EXIT_FAILURE);
	}
	...
	int speed_threshold = atoi(argv[4]);
	...
	std::cout << "# Speed threshold. " << speed_threshold << std::endl;
...
#pragma parrot.start("nrpol3_kernel")
	...
	nrpol3_kernel<<<dimGrid, dimBlock>>>(A_coeff_d, B_coeff_d, C_coeff_d, D_coeff_d, x0_d, root_d, data_size, err_thresh, speed_threshold);
