...
def calculate_err (array_1, array_2):
    ...
    for i in range(len(array_1)):
        ...
        if array_2[i]!=0:
            ...
            err += abs(array_1[i])
	abs_err += abs(array_1[i]- array_2[i])
        rmse = rmse + (array_1[i]- array_2[i])*(array_1[i]- array_2[i]);

    #print ("sum err %f "%(err))
    return err/float(len(array_1)), (rmse/float(len(array_1)))**0.5, abs_err
def main(argv):
    if (len(argv) != 2):
    ...
    err,rmse,abs_err = calculate_err (array_1, array_2)
    print ("err %f  rmse %f, total abs err %f"%(err,rmse, abs_err))
