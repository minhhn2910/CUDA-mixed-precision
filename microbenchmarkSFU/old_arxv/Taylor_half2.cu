...
__global__ void Vec_add(FP_DEV_TYPE x[], FP_DEV_TYPE y[], FP_DEV_TYPE z[], int n,  FP_DEV_TYPE lookup[], uint32_t startClk[], uint32_t stopClk[]) {
   ...
   half2 coeff_0 = __float2half2_rn(1);
   half2 coeff_2 = __float2half2_rn(0.2);
   half2 coeff_3 = __float2half2_rn(0.8);

   if (tid < n) {

     half2 temp = __float2half2_rn(x[tid]);
   ...
   sink = temp*coeff_2 ;
    half2 temp_2 = temp*temp;
    sink.x = sink.x + temp.x*coeff_3.x;
	sink.y = sink.y + temp.y*coeff_3.x;
    // stop timing
    uint32_t stop = 0;
    ...
    stopClk[tid] = stop;
  //  dsink[tid] = sink;
    float2 temp_res = __half22float2(sink);
    z[tid] = temp_res.x + temp_res.y;
   }
}  /* Vec_add */
