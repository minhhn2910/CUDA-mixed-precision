...
void dump(double* variables, int nel, int nelr)
{
        ...
        double total_everything = 0;
        {
                std::ofstream file("density");
                //file << nel << " " << nelr << std::endl;
                for(int i = 0; i < nel; i++){
                     file << h_variables[i + VAR_DENSITY*nelr] << std::endl;
                      total_density += h_variables[i + VAR_DENSITY*nelr];
                }
        }
        printf("total density %.10f\n", total_density);
        total_everything += total_density;
        {
                ...
                for(int i = 0; i < nel; i++)
                {
                        for(int j = 0; j != NDIM; j++){
                                ...
                                total_everything += h_variables[i + (VAR_MOMENTUM+j)*nelr];
                        }
                        file << std::endl;
                }
        }
        ...
        double sum_energy = 0;
        {
                ...
                for(int i = 0; i < nel; i++){
                     file << h_variables[i + VAR_DENSITY_ENERGY*nelr] << std::endl;
                      sum_energy += h_variables[i + VAR_DENSITY_ENERGY*nelr];
                }
        }
        printf("total energy %.10f \n", sum_energy);;
        total_everything+= sum_energy;
        printf("sum_everything %.10f \n", total_everything);
        delete[] h_variables;
}
...
__device__ __host__ inline void compute_flux_contribution_float(float& density, float3& momentum, float& density_energy, float& pressure, float3& velocity, float3& fc_momentum_x, float3& fc_momentum_y, float3& fc_momentum_z, float3& fc_density_energy)
{
	fc_momentum_x.x = velocity.x*momentum.x + pressure;
	fc_momentum_x.y = velocity.x*momentum.y;
	fc_momentum_x.z = velocity.x*momentum.z;
...
__device__ inline void compute_velocity_float(float& density, float3& momentum, float3& velocity)
{
	velocity.x = momentum.x / density;
	velocity.y = momentum.y / density;
	velocity.z = momentum.z / density;
...
__device__ inline double compute_speed_sqd_float(float3& velocity)
{
	return velocity.x*velocity.x + velocity.y*velocity.y + velocity.z*velocity.z;
}
...
__device__ inline double compute_pressure_float(float& density, float& density_energy, float& speed_sqd)
{
...
__device__ inline float compute_speed_of_sound_float(float& density, float& pressure)
{
...
__global__ void cuda_compute_flux(int nelr, int* elements_surrounding_elements, double* normals, double* variables, double* fluxes)
{
	...
	if (true){
		...
		float3 velocity_i;             				compute_velocity_float(density_i, momentum_i, velocity_i);
		float speed_sqd_i                          = compute_speed_sqd_float(velocity_i);
		float speed_i                              = sqrtf(speed_sqd_i);
		float pressure_i                           = compute_pressure_float(density_i, density_energy_i, speed_sqd_i);
		float speed_of_sound_i                     = compute_speed_of_sound_float(density_i, pressure_i);
		float3 flux_contribution_i_momentum_x, flux_contribution_i_momentum_y, flux_contribution_i_momentum_z;
		float3 flux_contribution_i_density_energy;
		compute_flux_contribution_float(density_i, momentum_i, density_energy_i, pressure_i, velocity_i, flux_contribution_i_momentum_x, flux_contribution_i_momentum_y, flux_contribution_i_momentum_z, flux_contribution_i_density_energy);
		...
		for(j = 0; j < NNB; j++)
		{
			...
			if(nb >= 0) 	// a legitimate neighbor
			{
				...
				density_energy_nb = variables[nb + VAR_DENSITY_ENERGY*nelr];
													compute_velocity_float(density_nb, momentum_nb, velocity_nb);
				speed_sqd_nb                      = compute_speed_sqd_float(velocity_nb);
				pressure_nb                       = compute_pressure_float(density_nb, density_energy_nb, speed_sqd_nb);
				speed_of_sound_nb                 = compute_speed_of_sound_float(density_nb, pressure_nb);
				                                    compute_flux_contribution_float(density_nb, momentum_nb, density_energy_nb, pressure_nb, velocity_nb, flux_contribution_nb_momentum_x, flux_contribution_nb_momentum_y, flux_contribution_nb_momentum_z, flux_contribution_nb_density_energy);
