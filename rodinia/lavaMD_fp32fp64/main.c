...
int test_output(fp* fv_cpu, int num_space_elements, char *gold_file) {
  ...
  for (i = 0; i < 4 * num_space_elements; i+=4) {
    ...
    sqr_err1 += (gold_v - fv_cpu[i])*(gold_v - fv_cpu[i]);
    sqr_err2 += (gold_x - fv_cpu[i + 1])*(gold_x - fv_cpu[i + 1]);
    sqr_err3 += (gold_y - fv_cpu[i + 2])*(gold_y - fv_cpu[i + 2]);
    sqr_err4 += (gold_z - fv_cpu[i + 3])*(gold_z - fv_cpu[i + 3]);
    if (gold_v != 0.0)
    ...
    if (gold_x != 0.0)
      avg_rlt_err += fabs((gold_x - fv_cpu[i + 1])/gold_x);
    if (gold_v != 0.0)
      avg_rlt_err +=  fabs((gold_v - fv_cpu[i])/gold_v);
    if (gold_y !=0.0)
      avg_rlt_err += fabs((gold_y - fv_cpu[i + 2])/gold_y);
    if (gold_z != 0.0)
      avg_rlt_err +=   fabs((gold_z - fv_cpu[i + 3])/gold_z);
 ...
 printf("rmse %lf %lf %lf %lf \n", sqrtf(sqr_err1/num_space_elements), sqrtf(sqr_err2/num_space_elements),sqrtf(sqr_err3/num_space_elements),sqrtf(sqr_err4/num_space_elements) );
 printf("%lf \n", sqrtf((sqr_err1+sqr_err2+sqr_err3+sqr_err4)/num_space_elements));
 printf("max rlt err %lf \n", max_rlt_err);
 printf("avg rlt err %lf \n", avg_rlt_err/(4 * num_space_elements));
  fclose(gold_fp);
  return 1;
}
