...
int test_output(fp* fv_cpu, int num_space_elements, char *gold_file) {
  ...
  for (i = 0; i < 4 * num_space_elements; i+=4) {
    ...
    rmse +=(gold_v - fv_cpu[i])*(gold_v - fv_cpu[i]);
    rmse += (gold_x - fv_cpu[i + 1])* (gold_x - fv_cpu[i + 1]);
    rmse += (gold_y - fv_cpu[i + 2])*(gold_y - fv_cpu[i + 2]);
    rmse += (gold_z - fv_cpu[i + 3])*(gold_z - fv_cpu[i + 3]);
...
printf("avg rlt err %lf  num_space_elements %d\n", avg_rlt_err/(float)(4 * num_space_elements), num_space_elements);
  printf("rmse %f \n", sqrtf(rmse/(float)(4 * num_space_elements)));
  fclose(gold_fp);
  return 1;
}
