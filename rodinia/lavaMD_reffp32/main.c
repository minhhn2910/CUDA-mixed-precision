...
#define OUTPUTCHECK
#ifdef OUTPUTCHECK
  FILE *gold_fp = fopen("gold_output.txt", "r");
 double max_rlt_err = 0.0;
  double avg_rlt_err = 0.0;
  double sqr_err = 0.0; 
  double sqr_err1 = 0.0;
 double sqr_err2 = 0.0;
double sqr_err3 = 0.0;
double sqr_err4 = 0.0;
  for (int j =0; j<5; j++) 
        printf("%f, %f, %f, %f\n", fv_cpu[j].v, fv_cpu[j].x, fv_cpu[j].y, fv_cpu[j].z) ;
  for (i = 0; i <  dim_cpu.space_elem;  i++) {
    ...
    if (gold_v != 0.0)
      if(fabs((gold_v - fv_cpu[i].v)/gold_v) > max_rlt_err)
		    max_rlt_err = fabs((gold_v - fv_cpu[i].v)/gold_v);
    if (gold_x != 0.0)
   	  if(fabs((gold_x - fv_cpu[i].x)/gold_x) > max_rlt_err)
		    max_rlt_err = fabs((gold_x - fv_cpu[i].x)/gold_x);
    if (gold_y !=0.0)
      if(fabs((gold_y - fv_cpu[i].y)/gold_y) > max_rlt_err)
		    max_rlt_err = fabs((gold_y - fv_cpu[i].y)/gold_y);
    if (gold_z != 0.0)
	   if(fabs((gold_z - fv_cpu[i].z)/gold_z) > max_rlt_err)
		   max_rlt_err = fabs((gold_z - fv_cpu[i].z)/gold_z);


//    if (i < 10)
 //       printf(" %f %f \n", fv_cpu[i].v, gold_v );
    sqr_err += (gold_v - fv_cpu[i].v)*(gold_v - fv_cpu[i].v);
    sqr_err += (gold_x - fv_cpu[i].x)*(gold_x - fv_cpu[i].x);
    sqr_err += (gold_y - fv_cpu[i].y)*(gold_y - fv_cpu[i].y);
    sqr_err += (gold_z - fv_cpu[i].z)*(gold_z - fv_cpu[i].z);
;
    sqr_err1 += (gold_v - fv_cpu[i].v)*(gold_v - fv_cpu[i].v);
    sqr_err2 += (gold_x - fv_cpu[i].x)*(gold_x - fv_cpu[i].x);
    sqr_err3 += (gold_y - fv_cpu[i].y)*(gold_y - fv_cpu[i].y);
    sqr_err4 += (gold_z - fv_cpu[i].z)*(gold_z - fv_cpu[i].z);
  }
printf("rmse %lf \n", sqrt(sqr_err/(4*dim_cpu.space_elem)));
printf("rmse %lf %lf %lf %lf\n", sqrt(sqr_err1/(dim_cpu.space_elem)),sqrt(sqr_err2/(dim_cpu.space_elem)),sqrt(sqr_err3/(dim_cpu.space_elem)),sqrt(sqr_err4/(dim_cpu.space_elem)));
 fclose(gold_fp);
